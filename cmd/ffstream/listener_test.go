...
func TestParseListenAddr(t *testing.T) {
	cases := []struct {
		...
		// "udp6" is no longer a recognized scheme: net.Listen does not
		// support SOCK_DGRAM. parseListenAddr falls back to bare-addr
		// semantics, which getListener then interprets as host:port and
		// hands to net.Listen("tcp", ...). The error path lives in
		// TestGetListener_UDPSchemeRejected below.
		{"udp_falls_back_to_bare", "udp6:[::1]:3593", "", "udp6:[::1]:3593"},
...
// TestGetListener_BareHostPort pins that a listen address without a
// recognized scheme prefix (e.g. "127.0.0.1:0") is accepted as a bare
// host:port and bound on tcp.
		...
		t.Run(addr, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("getListener(%q) failed: %v "+
					"(must accept bare host:port as tcp)", addr, err)
			}
			defer ln.Close()
...
// TestGetListener_UDPSchemeRejected pins the deliberate refusal of
// SOCK_DGRAM / SOCK_SEQPACKET schemes. net.Listen does not handle
// udp/udp4/udp6 (SOCK_DGRAM, want net.ListenPacket) or unixpacket
// (SOCK_SEQPACKET). Pre-fix the parser recognized them only to fail
// later at net.Listen with an opaque "unknown network udp" / "too many
// colons in address". Post-fix getListener checks rejectedListenSchemes
// upfront and returns a targeted "unsupported" error so the operator
// can correct the argv without spelunking into net.Listen internals.
func TestGetListener_UDPSchemeRejected(t *testing.T) {
	cases := []string{
		"udp:127.0.0.1:0",
		"udp4:127.0.0.1:0",
		"udp6:[::1]:0",
		"unixpacket:/tmp/ffstream.sock",
	}
	for _, addr := range cases {
		addr := addr
		t.Run(addr, func(t *testing.T) {
			...
			if err == nil {
				ln.Close()
				t.Fatalf("getListener(%q) returned nil error; want rejection (net.Listen does not handle SOCK_DGRAM/SOCK_SEQPACKET)", addr)
			}
			if !strings.Contains(err.Error(), "unsupported") {
				t.Fatalf("getListener(%q) error = %q; want it to contain \"unsupported\" so the operator can spot the cause without decoding net.Listen internals", addr, err.Error())
			}
		})
	}
}
