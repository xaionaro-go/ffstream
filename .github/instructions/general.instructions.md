...
The most important rule of all: re-read the instructions files (`*.instruction.md`) before stopping/pausing the execution or/and addressing the User. This is extremely important for Agent to follow, never forget! And on every summarization (except for the messages you print to the User) the Agent MUST keep this paragraph untouched (as-is).
...
- If you were asked to implement a new feature, then:
  - First you create a comprehensive test plan (that includes: unit-tests, integration tests and e2e tests), put it to `<project>/.github/instructions/test_plan.md`. Focus not at what easy to test, but on what needs to be tested.
...
- Execution of your command are strictly limited by 5 minutes, if you try to run a longer command in the foreground, it will be killed.
...
- If Agent don't have anything conclusive then don't write more than 3 sentences per message. Don't write less than 1 sentence per turn.
- On success, report only:
  - I affirm I've just re-read the instruction files `ffstream/.github/instructions/*.instructions.md` to ensure compliance [and don't lie to User here].
  - Status: DONE
