// hot_add_race_test.go regresses task #156: a hot AddInput at an
// already-active priority races with the in-flight transcoding path
// through the chain's MapStreamIndices and trips an assertion in
// newOutputStream (or surfaces a data race the -race detector
// flags). The reproduction wires a real lavfi-driven input chain,
// lets it serve, then issues many AddInput calls back-to-back so the
// Pause+Unpause hot-reload sequence collides with concurrent
// kernel.Generate / MapStreamIndices.SendInput work.
...
// window in which task #156's panic was observed.
//
...
func TestAddInput_HotReloadRace_NoPanic_NoRace(t *testing.T) {
	...
	// per the existing test, but defeats the repro window for #156).
