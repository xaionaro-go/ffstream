...
// TestBoundedSendInputUnion_ErrPipelineBusyOnFullChan pins that the
// bounded-send helper that InjectSubtitles delegates to must NOT block
// forever when the destination channel cannot accept the item. It must
// time out and return ErrPipelineBusy, and must invoke the abort
// cleanup exactly once so the caller's C-owned resources (packet pool
// entry, codec parameters) are released.
//
