...
// versionJSONPayload is the curated subset of BuildInfo emitted by
// --version. It is intentionally smaller than BuildInfo so the rendered
// JSON is byte-for-byte identical across binaries built from the same
// source tree at the same commit (e.g. ffstream and ffstreamctl):
//
//   - debug.BuildInfo.Main.Path differs per binary (cmd/ffstream vs
//     cmd/ffstreamctl) → excluded.
//   - debug.BuildInfo.Deps differs because each cmd pulls a different
//     subgraph of the module → excluded.
//   - vcs.* settings and BuildVars are stamped from the same VCS
//     working tree and the same linker flags → included.
//
// The included vcs settings cover everything observable about the
// source identity: revision, time, modified, plus the toolchain
// (GOOS/GOARCH/CGO_ENABLED/-buildmode/...) so build-environment drift
// remains visible.
type versionJSONPayload struct {
	BuildVars   *BuildVars        `json:",omitempty"`
	VCSSettings map[string]string `json:",omitempty"`
}

// versionJSONIncludedSettings is the allowlist of debug.BuildInfo
// settings rendered into versionJSONPayload. Anything outside this set
// is excluded — explicitly so. Adding a key here is a deliberate
// promotion that must keep the cross-binary identity invariant.
var versionJSONIncludedSettings = []string{
	"vcs",
	"vcs.revision",
	"vcs.time",
	"vcs.modified",
...
// versionPayload assembles the curated payload from a full BuildInfo.
func versionPayload(bi BuildInfo) versionJSONPayload {
	out := versionJSONPayload{BuildVars: bi.BuildVars}
	if bi.BuildInfo == nil {
		return out
	}
	settings := map[string]string{}
	for _, key := range versionJSONIncludedSettings {
		if v := bi.FindBuildInfoSetting(key); v != "" {
			settings[key] = v
		}
	}
	if len(settings) > 0 {
		out.VCSSettings = settings
	}
	return out
}
...
// so callers can write the buffer straight to stdout. The output is
// restricted to fields that are guaranteed identical between binaries
// built from the same source tree at the same commit; see
// versionJSONPayload for the rationale.
func JSON() ([]byte, error) {
	var buf bytes.Buffer
	enc := json.NewEncoder(&buf)
	enc.SetIndent("", " ")
	if err := enc.Encode(versionPayload(Get())); err != nil {
