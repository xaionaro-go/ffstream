...
# Cap Go's heap-size target. The runtime keeps RSS below this softly via
# more aggressive GC; values above the device's physical RAM defeat the
# cap because GC never feels memory pressure from this knob alone. Pixel-
# 8a-class devices have ~7.5 GB total RAM; 2 GiB lets the camera
# pipeline grow without crowding wingout + system services. Override via
# FFSTREAM_GOMEMLIMIT for higher-memory devices.
export GOMEMLIMIT="${FFSTREAM_GOMEMLIMIT:-2GiB}"

# Bias Android's low-memory killer (LMKD) to kill this daemon BEFORE
# wingout / system processes when memory pressure rises.
#
# AOSP's LMKD `find_and_kill_process()` iterates oom_score_adj buckets
# DESCENDING from OOM_SCORE_ADJ_MAX (1000) down to the tier's
# min_score_adj, INCLUSIVE on both ends, killing one process per call
# from the first non-empty bucket reached. Setting this daemon's
# oom_score_adj to OOM_SCORE_ADJ_MAX (1000) places it in the FIRST
# iteration bucket so it dies ahead of every system / app process at
# lower adj. Source: AOSP `system/memory/lmkd/lmkd.cpp`
# android16-release: `for (i = OOM_SCORE_ADJ_MAX; i >= min_score_adj;
# i--) { ... if (killed_size) break; }`.
#
# Per-device tier configuration is read from `sys.lmk.minfree_levels`
# (kb_threshold:adj_threshold pairs). On a Pixel-8a-class target,
# `getprop sys.lmk.minfree_levels` reports
# `18432:0,23040:100,27648:200,32256:250,55296:900,80640:950` — the
# top tier fires when free<80640KB and sweeps adj=1000 → adj=950
# descending. Empirically the bucket-1000 slot is empty by default on
# stock Pixel 8a while buckets 905-985 contain ~35 system / framework
# processes, so adj=1000 makes this daemon the unambiguous first-kill
# target whenever the top tier fires (this was already the value
# inherited from the Termux supervisor; an earlier 950 deployment
# inverted the kill order under that distribution).
#
# Verify on a different device by running
...
# Cap output-socket TCP MSS at 1200 bytes pre-connect via libav's
# `tcp_mss` AVOption (the `-tcp_mss` CLI flag below routes through
# Config.TCPMSS → pkg/ffstream/sender_factory.go ensureTCPMSS, per
# Task #166). Without this cap, libav's avio_open fires its 10s
# OpenTimeout on every Activate because phone TCP segments at the
# negotiated MSS=1460 (1448-byte payload + 40-byte TCP/IP headers
# = 1488 bytes total) exceed the cached phone→dev path MTU of
# 1246 bytes and get silently dropped on the forward path; cached
# PMTU 1246 is per Task #163 §3 phone-side pcap forensic capture.
# 1200 stays comfortably below 1246 with margin for IP-options
# padding.
