...
# Cap Go's heap-size target. See run-ffstream-camera.sh for full
# rationale; same Pixel-8a-class device-RAM constraint applies here.
export GOMEMLIMIT="${FFSTREAM_GOMEMLIMIT:-2GiB}"
# Place this daemon at the top of the device's LMKD priority range
# so it dies BEFORE wingout/system processes under memory pressure.
# See run-ffstream-camera.sh for full LMKD-priority-range rationale
# (the Pixel-8a-class top tier is `80640:950` per
# `getprop sys.lmk.minfree_levels`; AOSP `find_and_kill_process()`
# iterates the tier DESCENDING from OOM_SCORE_ADJ_MAX=1000 INCLUSIVE,
# so adj=1000 is the first-kill bucket within the tier).
...
#
# Cap output-socket TCP MSS at 1200 bytes pre-connect; see
# run-ffstream-camera.sh for full forward-path PMTU rationale
# (cached phone→dev path MTU 1246 bytes per Task #163; 1200 stays
# comfortably below). The `-tcp_mss` CLI flag implementation lives
# in cmd/ffstream + pkg/ffstream/sender_factory.go per Task #166.
