...
| `FFSTREAM_GOMEMLIMIT` | `2GiB` | Exported to ffstream; Go runtime soft memory target |
...
Header rule: `oom_score_adj=950` is set unconditionally (kill-first OOM target inside the device's LMKD priority range; see the LMKD comment block in `run-ffstream-camera.sh` for full rationale + `sys.lmk.minfree_levels` evidence). `nice -n -15` and `taskset -c 6-8` are also unconditional.
